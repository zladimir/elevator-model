...
void handler_sigint(int s){
	if (condition == 1){
		puts("The button on the 0 floor is pressed");
	} else{
		puts("The button on the 0 floor is not pressed");
	}
...
void handler_elevator_arrival(int s){
	if (condition == 1){
		condition = 0;
		puts("The button on the 0 floor is not pressed, elevator is here");
	}
	return;
}
