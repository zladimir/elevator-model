...
void handler_sigint(int s){
	if (condition == 1){
		puts("The cabin button is pressed");
	} else{
		puts("The cabin button is not pressed");
	}
...
void handler_elevator_arrival(int s){
	if (condition == 1){
		condition = 0;
		puts("The cabin button is pressed is not pressed, elevator reached the floor");
	}
	return;
}
